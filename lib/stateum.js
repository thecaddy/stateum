...
exports["default"] = sm => {
  ...
  return obj => {
    ...
    obj.transitionStates = function () {
      return obj.getState.apply(this).then(state => {
        ...
        if (stateMachine.states[state].transitionStates) {
          return toPromise(stateMachine.states[state].transitionStates.apply((0, _lodash.extend)(this, {
            STATEUM: stateMachine.states[state]
    ...
    obj.transitionTo = function (to) {
      return obj.transitionStates.apply(this).then(ret => {
        let allow = false;
        ...
        return toPromise(stateMachine.states[ret.state].transitions[to].apply((0, _lodash.extend)(this, {
          STATEUM: stateMachine.states[ret.state]
        }))).then(resp => {
          delete resp.STATEUM;
          return resp;
        });
